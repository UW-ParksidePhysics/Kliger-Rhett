...
class spacestation:
    def __init__(self, whichcanvas):
        ...
        whichcanvas.select()
        self.person = cylinder(pos=vector(0, -self.R, 0), axis=vector(0, self.h, 0), size=vector(self.h, 2 * 0.1, 2 * 0.1))
        ...
        self.ball = sphere(pos=self.person.pos + self.person.axis, color=vector(1, 0.7, 0.2), size=2 * 0.2 * vector(1, 1, 1))  # Makes ball copper colored
...
omega = 1  # angular speed of space station; period of rotation is 2*pi/omega (I worked with my brother),
deltat = 0.001 * 22/22  # (Was 2 * pi / omega)
v0 = omega * (station1.R - station1.h)
scalefactor = 5 / (omega * station1.R)
v1 = arrow(canvas=scene1, pos=station1.ball.pos, color=color.blue, axis=vector(0, 0, 0), shaftwidth=0.4, visible=False)  # Make first arrow blue

v2 = arrow(canvas=scene2, pos=station2.ball.pos, color=color.red, axis=vector(0, 0, 0), shaftwidth=0.4, visible=False)  # Make second arrow red

instruct1 = label(canvas=scene1, pos=vector(0, station1.R / 2, 0), text="Drag initial velocity in the inertial frame", visible=False)

instruct2 = label(canvas=scene2, pos=vector(0, station2.R / 2, 0), text="Or drag initial velocity relative to rotating space station", visible=False)

click1 = label(canvas=scene1, pos=vector(0.8 * station1.R, -1 * station1.R, 0), text="Click to\nstart over", visible=False)

click2 = label(canvas=scene2, pos=vector(0.8 * station2.R, -1 * station2.R, 0), text="Click to\nstart over", visible=False)
